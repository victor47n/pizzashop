...
test('update profile successfully', async ({ page }) => {
  ...
  await page.getByLabel('Nome').fill('aw4ys Pizza')
  ...
  await expect(page.getByRole('button', { name: 'aw4ys Pizza' })).toBeVisible()
})
...
test('update profile error', async ({ page }) => {
  ...
  await page.getByLabel('Nome').fill('Another Pizza')
  await page.getByLabel('Descrição').fill('Another description')
